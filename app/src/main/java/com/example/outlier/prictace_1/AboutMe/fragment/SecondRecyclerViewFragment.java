...
public class SecondRecyclerViewFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        noItem.setText("您还没有添加任何酒店");
        noItem.setVisibility(View.GONE);

        final List<hotelItem> items = new ArrayList<>();
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                try {
                    ...
                    if (type.equals("jiudian")) {
                        hotelItem first = new hotelItem(picture, title);
                        items.add(first);
                    }
        ...
        //将景点的列表列出来
        final hotelAdapter adapter=new hotelAdapter(items);
        adapter.setOnItemLongClickListener(new hotelAdapter.OnItemLongClickListener() {
            @Override
